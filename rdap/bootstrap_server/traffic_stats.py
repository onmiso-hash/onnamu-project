...
def _summarize(days):
    ...
    조회_성패 = {"성공": 0, "실패": 0}
    조회_종류 = {}
    # 조회가 어디서 왔나 — 우리 조회 화면인가, 남의 프로그램이 곧장 부른 것인가.
    # 표시를 남기기 전(2026-09-07 이전)의 기록에는 칸 자체가 없어 '판정전'이 된다.
    조회_어디서 = {"우리화면": 0, "직접호출": 0, "판정전": 0}
    tld_표 = {}
    대상_사람 = {}
    대상_전체 = {}
    시간표 = {}
    ...
    for stamp, path in 파일들:
        ...
        with fp:
            for raw in fp:
                ...
                # 'w' 칸이 아예 없는 줄은 표시를 남기기 전의 기록이다. 0으로 보면
                # 옛 기록 전부가 '직접호출'로 몰려 숫자가 거짓말을 한다.
                if "w" not in row:
                    조회_어디서["판정전"] += 1
                elif row.get("w"):
                    조회_어디서["우리화면"] += 1
