...
## 📅 주요 히스토리
- 2026-09-27: Night Grove 둘째 밤 '안개 늪' 추가 (첫째 밤을 한 번 이기면 열림, 시작 화면 밤 고르기, 새 적 안개 유령·독두꺼비, 보스 늪의 여왕 두꺼비·안개 군주, 첫째 밤 수치·기록 칸은 그대로이고 둘째 밤 기록은 `night_grove_best_2`). 상세 내용은 `history/GAMES_NIGHT_GROVE_SWAMP_20260927.md` 참조.
- 2026-09-26: 타워 디펜스 게임 '성벽의 노래' 추가 및 로비 연동 (Phaser 3 빌드 결과를 `games/wallsong/`에 두고 `/wallsong/`으로 내준다, 세 장·열두 스테이지·보스 3종, 영웅 셋, 적 걷기·공격 그림). 상세 내용은 `history/GAMES_WALLSONG_20260926.md` 참조.
- 2026-09-25: 생존 액션 게임 'Night Grove(밤의 숲)' 추가 및 로비 연동 (10분 생존·레벨업 능력 선택, 무기 6종·보조 능력 6종, 적 5종과 보스 2종, Canvas 조명 연출, Web Audio 효과음, 모바일 가상 조이스틱). 상세 내용은 `history/GAMES_NIGHT_GROVE_20260925.md` 참조.
- 2026-09-23: **Chronicle AI Studio 장면 배경 사진 올리기 + 19금 전용 장면 6개** — 사이드바 🎬에서 장면을 고른 뒤 '배경 올리기'로 사진을 붙인다. 파일은 감정 업로드와 같은 통로(`kind:'scene'`, 서버가 지은 이름·png/jpg/webp만)로 저장하고, 주소는 **그 대화**의 `meta.scenes[sceneId].url`에만 적는다(인물 JSON 아님, PATCH 허용 칸은 `store.js` 한 표에 `scenes` 추가). 무대 배경은 대화 사진 → 표의 그림 → 그라데이션 순. 19금 전용 장면 6개는 `canAdult`이고 그 대화가 19금일 때만 목록에 나오고 서버도 그 조건으로만 받으며, 수위를 내리면 장면이 studio로 돌아간다(사용자 결정). 19금 장면 사진 파일은 `canAdult`에게만 내준다(일반 계정 404). 상세는 `history/TASK_20260923.md` 참조.
- 2026-09-23: **Chronicle AI Studio 대화별 장면(배경) + 무대** — 상단 인물 카드를 무대(장면 배경 + 감정 초상)로 바꾸고, 사이드바에서 '이 대화 장면'을 고른다(8개, 그림 없으면 그라데이션, 목록은 `studio/sceneRegistry.js` 한 표). 장면의 주인은 대화(`meta.sceneId`, 수위와 같은 규칙)이며 손으로만 바꾼다(sticky). 휴대폰에서는 무대를 접을 수 있다. 이 김에 대화 PATCH 허용 칸이 `server.js`·`store.js` 두 벌이던 것을 `store.js` 한 표로 합쳤고, `/data/` 주소가 스타일 없는 첫 화면을 띄우던 것을 404로 바꿨다. 상세는 `history/TASK_20260923.md` 참조.
- 2026-09-23: **Chronicle AI Studio 감정 5종 → 12종(+19금 전용 lust)** — 감정 목록을 `studio/emotionRegistry.js` 한 표로 모아(설정창 칸·AI 규칙·스키마 enum·대체 사슬·서버 업로드 검사가 모두 이것만 읽음) 하드코딩 배열·삼항 라벨을 없앴다. 그림이 없는 감정은 가까운 감정(crying→sad, love→happy 등)을 거쳐 normal로 떨어져 5장만 있는 옛 인물도 그대로 동작한다. 이미지 생성은 **빈 칸만** 만들고(예전엔 전부 비우고 다시 만듦), 생성본을 base64로 인물 JSON에 담던 것을 파일로 저장하게 바꿨다. lust는 `canAdult`+19금에서만. 업로드는 감정 id·파일 형식(png·jpg·webp)을 검사한다. 장면(배경)·무대는 2차. 상세는 `history/TASK_20260923.md` 참조.
- 2026-09-23: **Chronicle AI Studio가 모든 계정의 자료 파일을 로그인 없이 내주고 있던 구멍을 막음** — `express.static(__dirname)`이 폴더를 통째로 열고 빗장은 확장자 있는 요청을 통과시켜, 비로그인이 `/data/users/<아이디>/.../turns.jsonl`(19금 포함)·서버 소스·올린 그림을 받을 수 있었다(로컬 실측 200). '/data' 접두어 차단은 `/%64ata/...` 인코딩으로 뚫려(실측) 공개할 화면 파일 3개만 허용 목록으로 내주게 바꾸고, `/data/uploads`는 `protectFiles` 빗장으로 로그인 뒤로 옮겼다(비로그인 401). 상세는 `history/TASK_20260923.md` 참조.
- **2026-09-07**: **조회 통계에서 '사람이 화면에서 한 조회'를 갈라냄** — "실제 조회 페이지에서 조회한 사람이 있나, 몇 건 했나"는 물음에 답할 수 없는 상태였다. 우리 조회 화면과 남의 프로그램이 **같은 주소를 부르고**(차이는 `?proxy=true` 하나), 미들웨어가 `request.url.path`만 넘겨 **물음표 뒤를 버리고 있었다** — 기록에 글자 하나 다르지 않게 남아 가를 방법이 원리적으로 없었다. 접속 기록에 `w` 칸을 신설해 우리 화면 조회를 표시하고(참으로 읽는 낱말을 FastAPI와 맞춤), 집계에 `조회.어디서`(우리화면·직접호출·**판정전**)를 더했다 — `w` 칸이 없는 옛 기록을 0으로 보면 전부 '직접호출'로 몰려 숫자가 거짓말을 한다(08-28 '알 수 없음'과 같은 이유). 곁들여 **`화면별` 자료는 이미 계산돼 내려오고 있었는데 대시보드가 안 그리고 있었다** — 화면마다 방문자 수를 함께 세게 하고 표를 붙였다. 쌍둥이(`traffic_log.py`·`visit_view.py` 포털·RDAP)를 함께 고쳤다. **오늘부터 쌓이는 기록만 갈린다.** 상세는 `history/TASK_20260907.md` 참조.
- 2026-08-28: **도메인 조회 통계를 기간별로, 사람과 기계를 갈라서 본다** — "오늘·7일·한달·전체로 뽑아보고 싶고, 봇의 훑기가 아닌 진짜 사람이 조회했는지, 했다면 어떤 주소를 조회했는지 알고 싶다"는 요청. 기존 화면이 읽던 것은 `manager.py`의 **시각 없는 누적 합계**라 기간을 나누는 것이 원리적으로 불가능했다. 대신 08-21에 만든 접속 기록(`/traffic/rdap-*.jsonl`)에서 뽑도록 `traffic_stats.py`를 신설했다. **착수 전에 실제로 세어 보니** 08-27 하루 3,295건 중 실패가 3,134건(95%)이고 도메인 조회 1,672건 가운데 1,669건이 `.kr`이었다 — 누적 합계로는 이 사실이 전혀 드러나지 않았다(8일치로는 조회 7,541건 중 성공 73건). 사람·기계를 가르려고 **접속 프로그램 이름표(User-Agent) 칸을 기록에 추가**했고 형식이 같아야 하는 세 곳을 함께 고쳤다. 판정은 두 축이다 — 무엇을 하려 했나(조회·자료·화면·훑기) × 누가(사람·프로그램·**알 수 없음**). '알 수 없음'을 따로 둔 것은 이름표를 오늘부터 기록하기 때문으로, 모르는 것을 기계로 몰면 실제 방문자가 사라지고 사람으로 치면 없는 방문자가 생긴다. **9월 3일 `.kr`·`.한국` 개통에 맞춰** 최상위 도메인 표를 성공·실패로 나누고 `xn--` 주소를 한글로 되돌려 표시한다. 접속 주소는 **로그인해야 원문이 보인다** — 다만 출입증 검사 규칙을 이 서버에 복사하지 않고 포털에 신분 확인 통로(`POST /api/auth/whoami`)를 열어 물어본다(규칙이 두 벌이 되는 것이 2026-08-18 갤러리 사고의 원인이었다). 사본 쿠키는 12시간만 산다. 집계는 60초 보관하고 화면 자동 갱신을 5초→30초로 늦췄다(8일치 1.7만 건 집계 0.14초 실측). 배포 전 로컬 도커로 조회 서버·포털을 함께 띄워 사람/프로그램/판정 전 구분, 한글 도메인 표시, 로그인 유무에 따른 주소 가림, 위조 출입증 거절까지 확인했다. 상세는 `history/TASK_20260828.md` 참조.
- 2026-08-21: **접속자 지도 보완 — 확대·도시 이름표·'유효한 요청만 보기'** — 대한민국을 지도 정중앙에 놓고(경도를 돌려 그린다. 접어 넣으면 양 끝 나라가 화면을 가로지르는 줄로 뭉개진다), 확대·이동(60배)과 도시 이름표를 넣었으며 지도 자료를 3km 정밀도(665KB·226개 나라)로 바꿨다. 설명 풍선은 브라우저 기본(`<title>`)이 1초 지연돼 '반응 없음'으로 읽히던 것을 직접 그려 즉시 뜨게 했다. **기록에 응답 코드를 추가**하고(기록 시점을 '요청 받을 때'→'답 내보낼 때'로 이동) 체크상자로 훑기·오류를 걸러 볼 수 있게 했다 — 판정은 ①훑기 경로 무조건 무효 ②응답 400 이상 무효 ③응답 코드 없으면 유효(모르는 것을 무효로 처리하면 멀쩡한 접속이 조용히 사라진다). 이 화면으로 **한 주소가 33초에 134개 경로를 두드린 php 훑기**를 실제로 잡았다(전부 404). 곁들여 확인: 도메인 조회는 `bootstrap.rdap.kr`이라 Cloudflare 위치 설정이 `onnamu.kr`과 **도메인마다 따로** 걸려 있었다. 상세는 `history/TASK_20260821.md` 참조.
- 2026-08-21: **접속자 지도 신설 — 어디서 어떤 주소가 얼마나 두드리는지 한 화면에서 본다** — "홈페이지와 RDAP에 유입 IP가 갑자기 늘었던 이력"이 발단. 관리자 전용 화면 `/admin/traffic`을 열었다. **착수 전에 먼저 쟀다** — 방문자의 접속 주소(`CF-Connecting-IP`)와 나라(`CF-IPCountry`)는 들어오지만 도시·위도·경도는 안 들어왔고(사용자가 Cloudflare 설정을 켠 뒤 들어오는 것을 재확인), 서버가 직접 보는 주소는 터널 자신의 것(`172.20.0.1`)이라 그대로 세면 방문자 전원이 한 사람으로 뭉친다. 포털·도메인 조회·크로니클 스튜디오 세 곳이 접속 한 건마다 같은 모양의 한 줄을 공용 보관함(`C:\Users\onmis\project\traffic`)에 **덧붙이기만** 한다(20줄/5초 묶음 — 같은 날 고친 '통째로 다시 쓰기' 결함을 되풀이하지 않기 위해). 화면에는 **시간대별 막대**(총량만 보면 순간 폭주가 평균에 묻힌다는 08-20 교훈을 구조에 반영) · 세계 지도(나라 색칠 + 접속 위치 점) · 나라별/주소별 표를 실었고, 지도 그림은 우리 서버에 담아 둔 공공 저작물(Natural Earth 110m, 205개 나라, 148KB)이라 **바깥 지도 서비스를 부르지 않는다.** 빗장은 계정 관리와 같은 것을 써서 30일짜리 출입증 표시만 믿지 않는다. 곁들여: 생존 확인 요청은 세지 않고, Cloudflare를 안 거친 직접 접속은 'direct'로 표시해 남긴다(포털 5001·조회 5004가 공유기에 열려 있어 지금까지 안 보이던 길). **나무 클라우드는 빠져 있다** — 코드가 `namu-cloud-routing` 방에 살아 그 방에 카드(`namu-cloud-traffic-log`)를 만들었다. 상세는 `history/TASK_20260821.md` 참조.
- 2026-08-20: **재생 화면이 폰·클라우드플레어에 30일 얼어붙어 영상이 느린 길로 다니고 있었다** — "미니PC CPU 100%, 어제까지 빠르던 영상이 오늘 느리다"는 신고. CPU만 보면 만성 부담(Celeron 3865U 2코어, 가상머신에 코어 1개만 배정, 도커 자체가 상시 23% 소모)이 먼저 잡히지만 **어제도 오늘도 같던 것이라 시점 차이를 설명하지 못했다.** 실측으로 갈랐다 — 직행 통로(`stream.onnamu.kr`) 통행이 **08-17 저녁 이후 0건**인데 통로 자체는 멀쩡(0.065초 응답), 재생 화면 요청도 **27시간 0건**인데 영화는 `/media` 느린 길로 나감. 원인은 캐시 판정이 **"주소가 `.mp4`로 끝나는가"**였던 것 — 재생 화면 주소가 `/player/영화이름.mp4`라 **화면(HTML)까지 30일 immutable**로 얼었다(두드려 확인: `.mp4`로 끝나면 `max-age=2592000, immutable`, 확장자만 떼면 `no-store`). 목록에서 카드를 눌러 들어가는 것은 새로고침이 아니라 평범한 이동이라 브라우저가 서버에 아예 묻지 않았고, 화면에 박힌 옛 주소(직행 통로 도입 전 `/media`)로 계속 영상이 나갔다. **클라우드플레어도 같은 이유로 이 화면을 캐시 대상(`cf-cache-status: MISS`)으로 잡고 있었다**(`/movies`는 `DYNAMIC`). 애초에 느린 주소가 박힌 경로는 `STREAM_BASE_URL=${STREAM_BASE_URL:-}` — 값이 빠져도 **조용히** 느린 길로 내려앉는 설정이었다. 고친 것: 캐시 판정을 확장자가 아닌 **경로 앞부분**(`/media/`·`/thumbnail/`·`/subtitle/`)으로 바꾸고 규칙을 `gallery/cache_policy.py` 한 벌로 분리(로그인 안내 302·오류 404도 얼리지 않도록 200/206/304로 좁힘), 재생 주소를 **`/player/…` → `/watch/…`**로 바꿔 이미 얼어붙은 화면을 즉시 버리게 함(옛 주소는 새 주소로 보냄), 직행 통로 값이 비면 **배포를 막음**(`:?`), 썸네일 판번호(`THUMB_VERSION`) 신설, 막는 검사 `scripts/check_gallery_cache.py`(push 전 자동, 일부러 깨뜨려 4종 모두 막히는 것 확인). **안 고친 것**: 갤러리 첫 화면 타일이 영상 원본을 통째로 받는 것(CPU 100%를 친 직접적 순간이 여기 — 화면 모양이 바뀌어 사용자 판단 보류), `/media`·영상 원본의 30일 캐시. 상세는 `history/TASK_20260820.md` 참조.
- 2026-08-18: **Chronicle AI 빗장 해제 + 19금을 관리자 표시에서 분리**(계정관리 4묶음, 설계 마지막 묶음) — `isAdmin` 한 값이 (가)문지기와 (나)19금 열람을 겸하고 있었다. (가)를 없애 로그인한 사람은 누구나 자기 자료로 들어오게 하고, (나)는 `canAdult`로 갈랐다(관리자는 자동 참). 세 파일의 `isAdmin` 49줄 → 7줄(남은 것은 관리용 통로 `DELETE /api/admin/user-data/:username`와 옛 화면 대비뿐), `canAdult` 51줄 신설. **19금 가리기 갈래는 빗장 때문에 배포 이래 한 번도 실행된 적이 없어** 시험 계정으로 실측했다 — 19금 인물이 목록에서 사라지고, 19금 대화는 주소로 직접 두드려도 **404**(감춤과 없음이 구분되지 않음), 대화가 감춰지는 두 갈래(대화 자체가 19금 · 19금 인물에 붙은 보통 대화) 모두 404, 감춰진 19금 자료는 저장으로 **지워지지 않고**, 19금 대화 새로 만들기는 403, 권한 변경은 **10초** 만에 반영. 배포 후 사용자가 잡은 결함 1건 — 빗장은 스튜디오에서 풀렸는데 **포털 첫 화면의 바로가기가 `{% if is_admin %}`에 감싸여 있어** 일반 계정에는 링크가 안 보였다(설계서의 '손대는 파일'에 이 줄이 없었다). 같은 종류의 잔존은 이 한 곳뿐임을 확인(갤러리는 3묶음에서 이미 `can_upload`/`is_admin`으로 갈라짐). 상세는 `history/TASK_20260818.md` 참조.
- 2026-08-18: **권한을 30일 출입증에서 떼어 포털에 물어보게 함**(계정관리 3묶음, 설계의 갈래 '가') — 권한이 출입증에 새겨져 있어 폴더를 빼도·잠가도·지워도 갤러리와 Chronicle AI에서는 최대 30일간 옛 권한이 살아 있었다. 이제 출입증은 '누구인가'만 담고, '무엇을 할 수 있는가'는 각 서비스가 포털의 `GET /api/auth/permissions/<아이디>`에 물어 온다(60초 캐시, `SECRET_KEY`를 열쇠로 받는 서비스 전용 통로). **반영이 최대 30일 → 1분으로** 줄었다. 포털이 멈추면 마지막에 들은 답으로 버티고, 그것조차 없으면 출입증 값으로 버티되 새 권한(올리기·19금)은 꺼진 쪽으로 둔다. 갤러리 올리기는 `can_upload`가 켜지고 **public이 아닌 폴더가 하나라도 있을 때만** 열리게 해 설계서가 실측으로 짚은 함정 2개(public만 가진 계정이 올리면 결국 public행 · 폴더 없는 계정이 올리면 `folders[0]`이 빈 목록을 집어 500)를 도달 불가로 만들었고, 올리기 단추가 `is_admin` 안쪽에 있어 허락받은 일반 계정에게 안 보이던 것도 고쳤다. 검사 42건 신설 + 1·2묶음 88건 재실행 통과, 배포 후 실물 25건(잠근 계정이 갤러리에서 1분 안에 막히는 것을 실제로 기다려 확인). 상세는 `history/TASK_20260818.md` 참조.
...
- **2026-08-20**: Chronicle AI가 휴대폰에서 대화 목록을 못 보여주고 늘 새 대화로 시작하던 것을
  ...
  `GET|PUT /api/studio-state`와 `studio-state.json`을 만들어 자리를 서버에 둔다.
  상세: `history/TASK_20260820.md`
- **2026-08-19**: 신분·권한 판정을 `shared/auth_common.py` 한 벌로 모음. 포털 관리자 통로 4곳이
  30일 묵은 출입증의 관리자 표시를 믿던 것을 지금 표를 보게 고쳤고, 기계 출입증(`system_`)이
  스튜디오에서 401로 막혀 계정 삭제 시 자료 정리가 실패하던 것을 함께 고쳤다.
  push를 막는 공통 검사 15개 추가. 상세: `history/TASK_20260819.md`
