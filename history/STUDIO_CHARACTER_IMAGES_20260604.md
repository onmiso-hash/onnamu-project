...
## 🚀 11. 추가 사항: 무부하 이벤트 기반 기기 간 동기화 및 500ms 전송 디바운스 적용 (2026-06-05)
* **이슈**:
  1. 기기 간(모바일 ↔ 노트북) 세션 실시간 백업이 제대로 작동하지 않아 모바일에서 명시적으로 저장하지 않은 경우 노트북에서 최신 대화가 유실되어 보이는 문제가 있었습니다.
  2. 서버로 동기화를 전송할 때 짧은 주기로 쓰기 요청이 몰릴 경우 파일 충돌(Race Condition) 리스크가 존재했습니다.
  3. 실시간 폴링을 도입할 경우 미니 PC의 디스크 부하 및 모바일 배터리/네트워크 대역폭 낭비가 우려되었습니다.
* **해결 조치**:
  1. **이중화 자동 저장 및 프리셋 예외 방어 구현**:
     * `generateNextChatNode`에서 비동기 RAG 임베딩의 수신 성공 여부와 상관없이 AI 대화 응답 수신 성공 직후 즉시 1차로 로컬스토리지에 저장하고 서버로 전송(`pushPersonasToServer()`)하도록 구조를 변경했습니다.
     * 프리셋을 수동으로 입력해 지정하지 않은 경우에도 격리된 임시 프리셋 세션(`_temp_anonymous_[캐릭터이름]`)으로 데이터를 격리 보존하여 기존 사용자 프리셋 데이터를 안전하게 방어하고 새로고침 복구도 완벽하게 지원합니다.
  2. **무부하 이벤트 기반 동기화 도입**:
     * 주기적으로 계속 요청을 쏘는 폴링 대신, 브라우저 탭에 포커스를 두거나 활성화하는 순간(`window.focus` 및 `visibilitychange` 이벤트)과 대화 입력창을 클릭하는 순간(`textarea.focus`)에만 서버로부터 최신 페르소나 정보를 받아와 머지하고 변경 시 챗 피드를 즉각 리렌더링하도록 구현했습니다. 이로써 네트워크와 서버 리소스 낭비는 **0%**가 되었습니다.
  3. **500ms 전송 디바운스(Debounce) 적용**:
     * 클라이언트에서 서버로 데이터를 POST 전송하는 `pushPersonasToServer` 메서드에 500ms 디바운스 필터를 적용하여 짧은 시간 내에 연달아 일어나는 파일 쓰기 충돌을 원천 차단했습니다.
