...
- **`server.js`**: Express 백엔드. Gemini API CORS 프록시. **로그인한 사람은 누구나 들어온다**(빗장 해제, 2026-08-18).
  ...
  - 보던 자리: `GET|PUT /api/studio-state` — **어느 대화를 열어 두었나만.** 대화 내용은 싣지 않는다.
...
- **`sceneRegistry.js`**: 장면(배경) 목록 **한 표**(8개, 그림 없으면 그라데이션). 무대 배경·사이드바 '이 대화 장면'·저장 검사가 여기만 읽는다.
  **장면의 주인은 대화다**(`meta.sceneId`, 수위와 같은 규칙). 새 대화는 `studio`(은은한 단색 배경), 옛 대화(키 없음)도 `studio`로 본다.
  초상은 **모든 장면에서 같은 3:4 세운 그림**이다 — 장면을 바꿔도 초상 모양·크기가 흔들리지 않게(2026-09-23 사용자 결정).
  ...
  **19금 전용 장면**(`adult: true`, 6개)은 `canAdult`이고 **그 대화가 19금**일 때만 고른다 — 화면 목록·서버 저장 검사가 모두
  `sceneAllowed` 하나로 판정한다. 수위를 내려 쓸 수 없게 되면 서버가 같은 요청에서 장면을 `studio`로 되돌린다(사진 기록은 남김).
  **배경 사진**은 사이드바 '배경 올리기' → `/api/upload-image`(`kind:'scene'`, 파일만 만든다) → `PATCH {scenes}`로 **그 대화의**
  `meta.scenes[sceneId].url`에 적는다(인물 JSON에 넣지 않는다). 주소는 `uploaded_<시각>_<장면id>.(png|jpg|webp)` 꼴만 받는다(`isSceneUploadUrl`).
  무대 배경 우선순위: 대화에 올린 사진 → 표의 `image` → 그라데이션. 19금 장면 사진 파일은 `canAdult`에게만 준다(그 외 404).
...
- **마지막으로 보던 자리**: `data/users/<아이디>/studio-state.json`. 화면을 되살리는 데 필요한
  글자 값 13개만 담는다(`store.js`의 `STUDIO_STATE_KEYS` — 모르는 칸은 버린다).
  브라우저 기억창고 이름과의 짝은 `app.js`의 `studioStateFields()` **한 표에만** 있다 —
  두 벌로 복사하지 말 것(한쪽만 고쳐지는 것이 이 프로젝트의 반복 결함이다).
  로그인 확인 직후 `pullStudioState()`로 받고, 세션 시작·홈 복귀·대화 전환 때 `pushStudioState()`로 적는다.
  **서버가 정본이되 서버에 값이 없으면 이 기기 것을 그대로 둔다** — 서버가 잠깐 답을 못 준
  순간에 이 기기의 자리까지 날리지 않기 위해서다.
  받아오는 자리는 **19금 복구 차단 필터보다 먼저** 와야 한다(그 필터가 읽는 값을 여기서 채운다).
- **서랍 안 항목은 줄어들지 않는다**(`flex-shrink: 0`). 이게 없으면 화면이 짧은 휴대폰에서
  대화 목록만 높이 0으로 눌려 통째로 사라진다 — 버튼은 글자 높이가 최소 크기라 버티는데
  목록은 안쪽 스크롤 때문에 최소 크기가 0이기 때문이다. 자리가 모자라면 **서랍이 통째로 스크롤된다.**
- **`index.html`의 `?v=` 표시**: `style.css`·`app.js`를 고치면 **반드시 함께 올린다.**
  안 올리면 폰과 클라우드플레어가 옛 파일을 계속 물고 있어 고친 것이 아무 데도 안 나타난다.
...
- **인증**: `authHelper.js`로 포털 SSO 토큰 검증. **출입증은 '누구인가'만 담는다** —
  ...
  **`system_`으로 시작하는 아이디는 기계 출입증이라 권한을 묻지 않고 통과시킨다**(2026-08-19).
  포털이 계정을 지우며 여기 자료 정리를 시킬 때 `system_portal`로 오는데, 묻던 시절에는
  계정표에 없어 401로 막혀 정리가 조용히 실패했다. 규칙의 원본은 `shared/auth_common.py`의
  `is_machine_identity`이고, 동작이 같은지는 `scripts/check_identity_common.py`가 잰다.
