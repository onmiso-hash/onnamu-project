...
> **2026-08-16 경위** — 이 계정은 한때 토큰 생성이 `Please verify your email (1211)`로 막혀 있었다.
> 공식 안내문의 제한 목록에는 토큰이 없지만 **실제로는 막힌다.** 확인 메일은 정상적으로 오고 있었고
> (발송 차단 목록이 아니었다), 만료 전에 열지 못한 것이 원인이었다. 메일 인증을 마치자 ①이 바로 풀렸다.
> **자격이 막혔을 때 "메일이 안 온다"를 발송 문제로 단정하지 말 것** — 만료가 더 흔하다.

### ③ 둘 다 막혔을 때 — 손으로 받기 (비상용, 지금은 쓰지 않는다)

인증서를 손으로 받는 길. **미니PC에서** 한 번 실행:
