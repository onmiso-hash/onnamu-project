...
- `curl -s -o /dev/null -w "%{http_code}" https://namu.onnamu.kr/`가 **404면 정상**
  ...
  - **502를 한 번 봤다고 실패로 단정하지 말 것.** `compose up -d`가 컨테이너를 교체하는
    ...
    못 붙었다"). 2026-07-25 v0.1.35 실측: push 100초 후 502 → 재확인 시 404 404 404,
    실제로는 정상 배포였다. 502·타임아웃이 나오면 **간격을 두고 3회 재확인**해
    값이 안정됐는지 보고 나서 판정한다.
  - 502가 계속되면 **대조군을 같이 재서 범위를 가른다** — `https://onnamu.kr/`(200 기대)와
    `https://namu-cloud.onnamu.kr/`(404 기대)를 함께 찍어, 터널·미니PC 전체 문제인지
    이 서비스 하나의 문제인지 구분한 뒤 보고한다.
