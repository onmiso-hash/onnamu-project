...
* **업로드 다이렉트 우회 채널**:
  * **외부 포트**: `50002` ──> **내부 매핑**: `172.30.1.100` (포트 `5444` / TCP)
  * **접속 주소**: `https://stream.onnamu.kr:50002` — **암호화 전용**. 평문으로 두드리면 400이 돌아온다.
  * **목적**: Cloudflare의 단일 파일 100MB 업로드 크기 제한을 우회하기 위한 다이렉트 통로.
  * **변경 사항 (2026-08-23)**: 이 자리에 갤러리 앱(Flask 개발용 서버)이 **평문으로 직접 노출**돼 있던 것을
    nginx(`stream` 스택, 5444 포트)가 앞에서 받도록 바꿨다. 인증서는 영상 직행 채널이 이미 받아 자동
    갱신하는 `stream.onnamu.kr` 것을 함께 쓴다. 갤러리에는 `ProxyFix(x_proto=1)`를 넣었다 —
    없으면 `request.url`이 평문으로 잡혀 로그인 복귀 주소가 깨진다.
    표준 443 포트로 옮기는 안은 **스캐닝 노출 증가를 이유로 사용자가 보류**했다(회선이 443 인바운드를
    막지 않는다는 것은 외부에서 실측 확인해 둠).
...
포털(`https://onnamu.kr`)과 직접 우회 채널 간의 오리진 불일치로 브라우저가 `.onnamu.kr` 쿠키를 실어 주지 않으므로 아래의 SSO 연동 방식을 사용합니다.

> **2026-08-23 정정**: 이 절은 우회 채널을 `http://gallery.onnamu.kr:50002`로 적고 있었으나 **그 주소는 닿지 않는다** —
> `gallery.onnamu.kr`는 Cloudflare 프록시로 해석되어 50002를 서비스하지 않는다. 실제 통로는
> `https://stream.onnamu.kr:50002`(2026-08-23 이후, 그 전에는 홈 공인 IP 직접)다.
> 스킴은 이제 양쪽 다 HTTPS이므로 '크로스 스킴'은 해소됐고, 남은 것은 **호스트·포트가 달라 쿠키가 공유되지 않는** 문제다.
