...
| **`namu-cloud-routing`** | `namu-cloud-routing:v0.1.9` (clone --recurse-submodules 로컬 빌드) | `0.0.0.0:8770` | `8770` | NAMU 공용 라우팅 MCP 서버 (`?user=<키>` per-user 격리, `namu_cloud_store` 볼륨에 사용자별 저장소 사본 / `namu_cloud_identity` 볼륨에 가입자 장부 `identity.db` 영속화, `/auth/github/*` 웹 로그인) |
